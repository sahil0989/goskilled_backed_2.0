...
const handleWebhook = async (req, res) => {
  try {
    ...
    if (parseFloat(amountFromWebhook) !== parseFloat(paymentRecord.amount).toFixed(2)) {
      console.warn(`⚠️ Amount mismatch for order ${orderId}: expected ${paymentRecord.amount}, got ${amountFromWebhook}`);
      // You may want to reject or handle specially
      // But you likely still want to proceed only with caution
    }
