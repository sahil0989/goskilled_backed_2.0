...
const handleWebhook = async (req, res) => {
    try {
        ...
        const rawBodyBuffer = req.rawBody; // Buffer from express.json verify
        ...
        const amountFromWebhook = eventData.data?.order?.order_amount;

        if (!orderId || !txStatus) {
            console.warn("⚠️ Invalid webhook payload: missing fields");
        ...
        // Amount verification
        const expectedAmount = Number(parseFloat(paymentRecord.amount).toFixed(2));
        const receivedAmount = Number(parseFloat(amountFromWebhook).toFixed(2));

        if (expectedAmount !== receivedAmount) {
            console.warn(
                `⚠️ Amount mismatch for order ${orderId}: expected ${expectedAmount}, got ${receivedAmount}`
            );
            // You can decide whether to return 400 or continue with caution
        ...
        paymentRecord.status = txStatus.toUpperCase();
        paymentRecord.transactionId =
            eventData.data?.payment?.cf_payment_id || paymentRecord.transactionId;
        paymentRecord.paymentMethod =
            eventData.data?.payment?.payment_method?.payment_mode || paymentRecord.paymentMethod;
        paymentRecord.responseData = eventData;
        await paymentRecord.save();

        // Grant course/package access if successful
        if (["SUCCESS", "PAID"].includes(txStatus.toUpperCase())) {
            ...
            if (user) {
                ...
                user.purchasedCourses = Array.from(
                    new Set([...user.purchasedCourses, ...purchasedCourseIds])
                );
                user.enrolledCourses = Array.from(
                    new Set([...user.enrolledCourses, ...purchasedCourseIds])
                );
                ...
                await user.save();
                console.log(`✅ User ${user._id} updated with purchased courses/packages`);
            }
        }
